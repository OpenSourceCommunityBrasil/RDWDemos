...
Uses
  ...
  Forms, Dialogs, StdCtrls, UDWJSONObject, DB, Grids, DBGrids, 
  URESTDWBase,
  UDWJSONTools, 
  UDWConsts, 
  URESTDWPoolerDB,
...
Type
  TForm2 = Class(TForm)
    ...
    procedure RESTDWDataBase1FailOverExecute(ConnectionServer
      : TRESTDWConnectionServer);
    procedure RESTDWDataBase1FailOverError(ConnectionServer
      : TRESTDWConnectionServer; MessageError: string);
    procedure cbUseCriptoClick(Sender: TObject);
    procedure RESTDWClientSQL1WriterProcess(DataSet: TDataSet;
      RecNo, RecordCount: Integer; var AbortProcess: Boolean);
    ...
    procedure RESTDWTable1WriterProcess(DataSet: TDataSet;
      RecNo, RecordCount: Integer; var AbortProcess: Boolean);
...
Begin
  ...
  Try
    ...
    For I := 0 To vKeyFields.Count - 1 Do
      RESTDWClientSQL1.FindField(vKeyFields[I]).ProviderFlags :=
...
Begin
  ...
  Except
    On E: Exception Do
    Begin
      Raise Exception.Create('Erro ao executar a consulta: ' + sLineBreak +
        E.Message);
    End;
  End;
  FIM := Now;
...
Begin
  ...
  Begin
    If Not RESTDWClientSQL1.ExecSQL(VError) Then
      Application.MessageBox(PChar('Erro executando o comando ' +
        RESTDWClientSQL1.SQL.Text), 'Erro...', Mb_IconError + Mb_Ok)
    Else
      Application.MessageBox
        (PChar(Format('Comando executado com sucesso...Linhas Afetadas %d',
        [RESTDWClientSQL1.RowsAffected])), 'Informação !!!',
        Mb_iconinformation + Mb_Ok);
  End
  Else
  Begin
    RESTDWClientSQL1.ExecSQL;
    RESTDWDataBase1.ApplyUpdates(DWMassiveCache1, vErrorResult, VError);
    // RESTDWClientSQL1.ApplyUpdates;
  End;
  RESTDWClientSQL1.Active := Not RESTDWClientSQL1.Active;
  If not RESTDWClientSQL1.Active Then
    SetKeys;
End;
...
Begin
  ...
  Begin
    ...
    Begin
      ...
      ShowMessage(unescape_chars (dwParams.ItemsString['result'].AsString));
    End;
  End
  Else
  Begin
...
Begin
  ...
  begin
    GetLoginOptionsClientPooler;
    If (dwParams.ItemsString['result'] <> Nil) AND
       (dwParams.ItemsString['result'].AsString <> '') Then
...
Begin
  ...
  End;
  RESTClientPooler1.AuthenticationOptions.AuthorizationOption :=
    RESTDWDataBase1.AuthenticationOptions.AuthorizationOption;
  ...
  Begin
    ...
    Begin
      TRDWAuthOptionBearerClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).TokenRequestType :=
        rdwtRequest;
      TRDWAuthOptionBearerClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).Token :=
        eTokenID.Text;
      TRDWAuthOptionBearerClient
        (RESTClientPooler1.AuthenticationOptions.OptionParams).TokenRequestType
        := TRDWAuthOptionBearerClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).TokenRequestType;
      TRDWAuthOptionBearerClient
        (RESTClientPooler1.AuthenticationOptions.OptionParams).Token :=
        TRDWAuthOptionBearerClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).Token;
    End
    Else
    Begin
      TRDWAuthOptionTokenClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).TokenRequestType :=
        rdwtRequest;
      TRDWAuthOptionTokenClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).Token :=
    ...
    End;
  End
  Else If RESTDWDataBase1.AuthenticationOptions.AuthorizationOption =
    rdwAOBasic Then
  Begin
    TRDWAuthOptionBasic(RESTDWDataBase1.AuthenticationOptions.OptionParams)
      .Username := edUserNameDW.Text;
    TRDWAuthOptionBasic(RESTDWDataBase1.AuthenticationOptions.OptionParams)
      .Password := edPasswordDW.Text;
    TRDWAuthOptionBasic(RESTClientPooler1.AuthenticationOptions.OptionParams)
      .Username := TRDWAuthOptionBasic
      (RESTDWDataBase1.AuthenticationOptions.OptionParams).Username;
    TRDWAuthOptionBasic(RESTClientPooler1.AuthenticationOptions.OptionParams)
      .Password := TRDWAuthOptionBasic
      (RESTDWDataBase1.AuthenticationOptions.OptionParams).Password;
  End;
End;

Procedure TForm2.GetLoginOptionsDatabase;
Begin
  If RESTDWDataBase1.AuthenticationOptions.AuthorizationOption
    in [rdwAOBearer, rdwAOToken] Then
  Begin
    If RESTDWDataBase1.AuthenticationOptions.AuthorizationOption = rdwAOBearer Then
    Begin
      eTokenID.Text := TRDWAuthOptionBearerClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).Token;
      lTokenBegin.Caption := FormatDateTime('dd/mm/yyyy hh:mm:ss',
        TRDWAuthOptionBearerClient(RESTDWDataBase1.AuthenticationOptions.
        OptionParams).BeginTime);
      lTokenEnd.Caption := FormatDateTime('dd/mm/yyyy hh:mm:ss',
        TRDWAuthOptionBearerClient(RESTDWDataBase1.AuthenticationOptions.
        OptionParams).EndTime);
      vSecresString := TRDWAuthOptionBearerClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).Secrets;
    End
    Else
    Begin
      eTokenID.Text := TRDWAuthOptionTokenClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).Token;
      lTokenBegin.Caption := FormatDateTime('dd/mm/yyyy hh:mm:ss',
        TRDWAuthOptionTokenClient(RESTDWDataBase1.AuthenticationOptions.
        OptionParams).BeginTime);
      lTokenEnd.Caption := FormatDateTime('dd/mm/yyyy hh:mm:ss',
        TRDWAuthOptionTokenClient(RESTDWDataBase1.AuthenticationOptions.
        OptionParams).EndTime);
      vSecresString := TRDWAuthOptionTokenClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).Secrets;
    End;
  End;
End;
...
Begin
  If RESTClientPooler1.AuthenticationOptions.AuthorizationOption
    in [rdwAOBearer, rdwAOToken] Then
  Begin
    ...
    Begin
      eTokenID.Text := TRDWAuthOptionBearerClient
        (RESTClientPooler1.AuthenticationOptions.OptionParams).Token;
      lTokenBegin.Caption := FormatDateTime('dd/mm/yyyy hh:mm:ss',
        TRDWAuthOptionBearerClient(RESTClientPooler1.AuthenticationOptions.
        OptionParams).BeginTime);
      lTokenEnd.Caption := FormatDateTime('dd/mm/yyyy hh:mm:ss',
        TRDWAuthOptionBearerClient(RESTClientPooler1.AuthenticationOptions.
        OptionParams).EndTime);
      vSecresString := TRDWAuthOptionBearerClient
        (RESTClientPooler1.AuthenticationOptions.OptionParams).Secrets;
    End
    Else
    Begin
      eTokenID.Text := TRDWAuthOptionTokenClient
        (RESTClientPooler1.AuthenticationOptions.OptionParams).Token;
      lTokenBegin.Caption := FormatDateTime('dd/mm/yyyy hh:mm:ss',
        TRDWAuthOptionTokenClient(RESTClientPooler1.AuthenticationOptions.
        OptionParams).BeginTime);
      lTokenEnd.Caption := FormatDateTime('dd/mm/yyyy hh:mm:ss',
        TRDWAuthOptionTokenClient(RESTClientPooler1.AuthenticationOptions.
        OptionParams).EndTime);
...
Begin
  ...
  If RESTDWDataBase1.AuthenticationOptions.AuthorizationOption in [rdwAOBearer, rdwAOToken] Then
    If RESTDWDataBase1.AuthenticationOptions.AuthorizationOption = rdwAOBearer Then
      Result := TRDWAuthOptionBearerClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).Secrets
    Else
      Result := TRDWAuthOptionTokenClient
        (RESTDWDataBase1.AuthenticationOptions.OptionParams).Secrets;
End;
...
procedure TForm2.RESTDWClientSQL1WriterProcess(DataSet: TDataSet;
  RecNo, RecordCount: Integer; var AbortProcess: Boolean);
begin
...
Begin
  ...
  Else
    Memo1.Lines.Add(DateTimeToStr(Now) +
      ' - Falha de conexão ao Database: ' + Error);
End;

procedure TForm2.RESTDWDataBase1FailOverError(ConnectionServer
  : TRESTDWConnectionServer; MessageError: string);
begin
  Memo1.Lines.Add(Format('FailOver Error(Server %s) : ', [ConnectionServer.Name,
    MessageError]));
end;

procedure TForm2.RESTDWDataBase1FailOverExecute(ConnectionServer
  : TRESTDWConnectionServer);
begin
...
procedure TForm2.RESTDWTable1WriterProcess(DataSet: TDataSet;
  RecNo, RecordCount: Integer; var AbortProcess: Boolean);
begin
  If Assigned(ProgressBar1) Then
  Begin
