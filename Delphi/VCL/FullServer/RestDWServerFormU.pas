...
type
  TRestDWForm = class(TForm)
    ...
    procedure RESTDWIdServicePooler1LastRequest(Value: string);
    procedure RESTDWIdServicePooler1LastResponse(Value: string);
  Private
    { Private declarations }
    VLastRequest, VLastResponse, VDatabaseName, FCfgName, VDatabaseIP,
...
Uses
  Winapi.ShellApi, uDmService, uDmService2, uRESTDWConsts, DataUtils;
...
begin
  ...
  begin
    ...
    labConexao.Caption := ' .: SERVER CONFIGS';
    labDBConfig.Caption := ' .: SQL DEFINITIONS';
...
procedure TRestDWForm.RESTDWIdServicePooler1LastRequest(Value: string);
begin
...
procedure TRestDWForm.RESTDWIdServicePooler1LastResponse(Value: string);
begin
  TThread.CreateAnonymousThread(
    procedure
    begin
...
Begin
  ...
  RESTDWIdServicePooler1.AddDataRoute('datadm', TServerMethodDM);
  RESTDWIdServicePooler1.AddDataRoute('datadm2', TServerMethodDM2);
      ...
      rdwAOBearer, rdwAOToken:
        Begin
          ...
          Begin
            ...
            TRESTDWAuthOptionBearerServer
              (RESTDWIdServicePooler1.AuthenticationOptions.OptionParams)
              .TokenHash := eTokenHash.Text;
            TRESTDWAuthOptionBearerServer
              (RESTDWIdServicePooler1.AuthenticationOptions.OptionParams)
              .ServerSignature := eServerSignature.Text;
            TRESTDWAuthOptionBearerServer
              (RESTDWIdServicePooler1.AuthenticationOptions.OptionParams)
              .LifeCycle := StrToInt(eLifeCycle.Text);
          End
          Else
          Begin
...
var
  VTempLastRequest, VTempLastResponse: string;
Begin
  Tupdatelogs.Enabled := False;
  Try
    VTempLastRequest := VLastRequest;
    VTempLastResponse := VLastResponse;
    ...
    Begin
      ...
      If MemoResp.Lines.Count > 0 Then
        If MemoResp.Lines[MemoResp.Lines.Count - 1] = VTempLastResponse Then
          Exit;
      If MemoResp.Lines.Count = 0 Then
        MemoResp.Lines.Add(Copy(VTempLastResponse, 1, 100))
      Else
        MemoResp.Lines[MemoResp.Lines.Count - 1] :=
          Copy(VTempLastResponse, 1, 100);
      If Length(VTempLastRequest) > 1000 Then
        MemoResp.Lines[MemoResp.Lines.Count - 1] :=
          MemoResp.Lines[MemoResp.Lines.Count - 1] + '...';
    End;
