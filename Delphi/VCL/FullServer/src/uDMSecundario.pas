...
USES
  ...
  uRESTDWJSONObject, Dialogs, uRESTDWDataUtils, FireDAC.Dapt, FireDAC.Phys.FBDef,
...
TYPE
  TDMSecundario = CLASS(TServerMethodDataModule)
...
procedure TDMSecundario.employeeReplyEvent(var Params: TRESTDWParams; dJsonMode : TJsonMode; Var Result : String);
Var
...
procedure TDMSecundario.FDQuery1AfterScroll(DataSet: TDataSet);
begin
...
procedure TDMSecundario.DWServerContext1ContextListangularReplyRequest(
  const Params: TRESTDWParams; var ContentType, Result: string;
  const RequestType: TRequestType);
var
 s : TStringlist;
...
procedure TDMSecundario.DWServerContext1ContextListindexReplyRequest(
  const Params: TRESTDWParams; var ContentType, Result: string;
  const RequestType: TRequestType);
var
 s : TStringlist;
...
procedure TDMSecundario.DWServerContext1ContextListindexReplyRequestStream(
...
procedure TDMSecundario.DWServerContext1ContextListinitReplyRequest(
  const Params: TRESTDWParams; var ContentType, Result: string;
  const RequestType: TRequestType);
begin
 Result := '<!DOCTYPE html> ' +
...
procedure TDMSecundario.DWServerContext1ContextListopenfileReplyRequestStream(
  const Params: TRESTDWParams; var ContentType: string; const Result: TMemoryStream;
  const RequestType: TRequestType; var StatusCode: Integer);
Var
 vNotFound   : Boolean;
...
procedure TDMSecundario.DWServerContext1ContextListphpReplyRequest(
  const Params: TRESTDWParams; var ContentType, Result: string;
  const RequestType: TRequestType);
var
 s : TStringlist;
...
procedure TDMSecundario.DWServerContext1ContextListzsendReplyRequest(
  const Params: TRESTDWParams; var ContentType, Result: string;
  const RequestType: TRequestType);
var
 s : TStringlist;
...
procedure TDMSecundario.DWServerEvents1EventsassynceventReplyEvent(
  var Params: TRESTDWParams; var Result: string);
Var
...
procedure TDMSecundario.DWServerEvents1EventseventdatetimeReplyEvent(
  var Params: TRESTDWParams; var Result: string);
begin
 Params.ItemsString['result'].AsDateTime := Params.ItemsString['mydatetime'].AsDateTime + 1;
end;

procedure TDMSecundario.DWServerEvents1EventseventintReplyEvent(
  var Params: TRESTDWParams; var Result: string);
begin
 Params.ItemsString['result'].AsInteger := Random(Params.ItemsString['mynumber'].AsInteger);
end;

procedure TDMSecundario.DWServerEvents1EventsgetemployeeDWReplyEvent(
  var Params: TRESTDWParams; var Result: string);
begin
 employeeReplyEvent(Params, Params.JsonMode, Result);
end;

procedure TDMSecundario.DWServerEvents1EventsgetemployeeReplyEvent(
  var Params: TRESTDWParams; var Result: string);
Begin
 employeeReplyEvent(Params, Params.JsonMode, Result);
End;

procedure TDMSecundario.DWServerEvents1EventshelloworldRDWReplyEvent(
  var Params: TRESTDWParams; var Result: string);
begin
 Params.ItemsString['result'].AsString := 'Hello World : ' + QuotedSTR(Params.ItemsString['entrada'].AsString);
end;

procedure TDMSecundario.DWServerEvents1EventshelloworldReplyEvent(
  var Params: TRESTDWParams; var Result: string);
begin
...
procedure TDMSecundario.DWServerEvents2Eventshelloworld2ReplyEvent(
  var Params: TRESTDWParams; var Result: string);
begin
 Result := 'Sou eu ServerEvent2';
end;

procedure TDMSecundario.DWSETESTEEventsservertimeReplyEventByType(
...
Function TDMSecundario.GetGenID(GenName  : String): Integer;
Var
...
procedure TDMSecundario.RDWSEDadosEventshelloworldReplyEventByType(
  var Params: TRESTDWParams; var Result: string;
  const RequestType: TRequestType; var StatusCode: Integer;
  RequestHeader: TStringList);
begin
...
procedure TDMSecundario.RESTDWDriverFD1PrepareConnection(
  var ConnectionDefs: TConnectionDefs);
begin
 vConnectFromClient := True;
 ConnectionDefs.DatabaseName := IncludeTrailingPathDelimiter(fPrincipal.EdPasta.Text) + ConnectionDefs.DatabaseName;
...
procedure TDMSecundario.ServerMethodDataModuleCreate(Sender: TObject);
begin
 vConnectFromClient := False;
 RESTDWPoolerFD.Active  := fPrincipal.CbPoolerState.Checked;
end;

procedure TDMSecundario.ServerMethodDataModuleGetToken(Welcomemsg,
...
Var
 ...
 Begin
  ...
  Try
   {$IFDEF APPWIN}
   v404Error.LoadFromFile(fPrincipal.RESTServicePooler1.RootPath + Const404Page);
   {$ELSE}
...
procedure TDMSecundario.ServerMethodDataModuleMassiveProcess(
  var MassiveDataset: TMassiveDatasetBuffer; var Ignore: Boolean);
begin
...
Procedure TDMSecundario.ServerMethodDataModuleUserTokenAuth(Welcomemsg,
  AccessTag: string; Params: TRESTDWParams; AuthOptions: TRESTDWAuthTokenParam;
  Var ErrorCode: Integer; var ErrorMessage, TokenID: string;
  Var Accept: Boolean);
begin
...
PROCEDURE TDMSecundario.Server_FDConnectionBeforeConnect(Sender: TObject);
VAR
  Driver_BD: STRING;
  Porta_BD: STRING;
  Servidor_BD: STRING;
...
PROCEDURE TDMSecundario.Server_FDConnectionError(ASender, AInitiator: TObject; VAR AException: Exception);
BEGIN
