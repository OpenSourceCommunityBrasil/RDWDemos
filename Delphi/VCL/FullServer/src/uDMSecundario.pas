...
procedure TDMSecundario.RDWSEDadosEventshelloworldReplyEventByType
  ...
  RequestHeader: TStringList);
begin
  Result := 'Hello World RDW Refactor...' + sLineBreak + Format('Param %s = %s',
    ['temp', Params.ItemsString['temp'].AsString]);
end;

procedure TDMSecundario.RESTDWDriverFD1PrepareConnection(var ConnectionDefs
  : TConnectionDefs);
begin
...
procedure TDMSecundario.ServerMethodDataModuleMassiveProcess(var MassiveDataset
  : TMassiveDatasetBuffer; var Ignore: Boolean);
begin
...
PROCEDURE TDMSecundario.Server_FDConnectionError(ASender, AInitiator: TObject;
  VAR AException: Exception);
BEGIN
